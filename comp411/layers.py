...
def affine_forward(x, w, b):
    ...
    refactor = np.prod(x.shape[1:])
    X=x.reshape(x.shape[0],refactor)
    out = X @ w
    out += b
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    H_out = 1 + (H + 2 * int(p) - HH) // int(stride)
    ...
    for idx,entry in enumerate(x_padded):
        for idx2,filter in enumerate(w):
            for stride_h in range(H_out):
                for stride_w in range (W_out):
                    slided_window= entry[:, (stride_h*stride):(stride_h*stride+ HH), (stride_w*stride):(stride_w*stride+ WW)]
                    out[idx, idx2, stride_h, stride_w] = np.sum((slided_window * filter)) + b[idx2]
...
def conv_backward_naive(dout, cache):
    ...
    for idx,entry in enumerate(x_padded):
        for idx2,filter in enumerate(w):
            for stride_h in range(H_out):
                for stride_w in range (W_out):
                    dw[idx2,:,:,:] += entry[:, (stride_h*stride):(stride_h*stride+ HH), (stride_w*stride):(stride_w*stride+ WW)] * dout[idx, idx2, stride_h, stride_w]
                    db[idx2] += dout[idx, idx2, stride_h, stride_w]

                    dx[idx,:, (stride_h*stride):(stride_h*stride+ HH), (stride_w*stride):(stride_w*stride+ WW)] += filter *  dout[idx, idx2, stride_h, stride_w]
...
def max_pool_forward_naive(x, pool_param):
    ...
    for idx,entry in enumerate(x):
        for depth in range(C):
            for stride_h in range(H_out):
                for stride_w in range (W_out):
                    slided_window= entry[depth, (stride_h*stride):(stride_h*stride+ pool_height), (stride_w*stride):(stride_w*stride+ pool_width)]
                    out[idx,depth, stride_h, stride_w] = np.amax(slided_window)
    ...
    for idx,entry in enumerate(x):
        for depth in range(C):
            for stride_h in range(H_out):
                for stride_w in range (W_out):
                    slided_window= entry[depth, (stride_h*stride):(stride_h*stride+ pool_height), (stride_w*stride):(stride_w*stride+ pool_width)]
                    dx[idx ,depth, stride_h*stride + np.argmax(np.max(slided_window, axis=1)) , stride_w*stride+ np.argmax(np.max(slided_window, axis=0))] += 1*dout[idx,depth,stride_h,stride_w]
    ...
    for idx,entry in enumerate(x):
        for depth in range(C):
            for stride_h in range(H_out):
                for stride_w in range (W_out):
                    slided_window= entry[depth, (stride_h*stride):(stride_h*stride+ pool_height), (stride_w*stride):(stride_w*stride+ pool_width)]
                    out[idx,depth, stride_h, stride_w] = np.mean(slided_window)
    ...
    for idx,entry in enumerate(x):
        for depth in range(C):
            for stride_h in range(H_out):
                for stride_w in range (W_out):
                    slided_window= entry[depth, (stride_h*stride):(stride_h*stride+ pool_height), (stride_w*stride):(stride_w*stride+ pool_width)]
                    dx[idx,depth, (stride_h*stride):(stride_h*stride+ pool_height), (stride_w*stride):(stride_w*stride+ pool_width)] += 1* (dout[idx,depth, stride_h,stride_w])/(pool_height* pool_width)
